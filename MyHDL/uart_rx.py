...
@block
def uart_rx(i_Clock,i_RX_Serial,o_RX_DV,o_RX_Byte,c_CLKS_PER_BIT=None):
	...
	def recv():
		if(r_SM_Main==IDLE):
			...
			if (r_Clock_Count == (c_CLKS_PER_BIT -1)//2):
				if ( i_RX_Serial == 0):
					r_Clock_Count.next = 0
			...
			if (r_Clock_Count < c_CLKS_PER_BIT -1):
				r_Clock_Count.next = r_Clock_Count + 1
				r_SM_Main.next = RX_DATA_BITS
			else:
				r_Clock_Count.next = 0
...
def convert_uart(hdl):
	...
	uart_rx_inst = uart_rx(i_Clock,i_RX_Serial,o_RX_DV,o_RX_Byte,c_CLKS_PER_BIT=868)
